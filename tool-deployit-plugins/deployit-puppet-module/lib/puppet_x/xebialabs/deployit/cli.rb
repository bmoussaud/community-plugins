...
#module PuppetX::Xebialabs::Deployit

class DeployitCommunicator
  def initialize(endpoint, username, password, context)
    raise 'Deployit communicator endpoint is nil or empty' if endpoint.nil?
    raise 'Deployit communicator username is nil or empty' if username.nil?
    raise 'Deployit communicator password is nil or empty' if password.nil?
    ...
    @context=context
    @types={}
    @str = "Deployit Communicator #{endpoint}#{@context}";
  end
