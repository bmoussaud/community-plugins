...
deployit_version { "Applications/demo-app/2.0":
  ensure   => present,
  server   => Deployit["remote"],
  path     => "tests/dar/demo-app-2.0.dar"
...
deployed_application { "Puppet application is deployed on dev environment":
  ...
  server => Deployit['remote'],
  require => [Deployit_container['Infrastructure/local.host'],Deployit_version["Applications/demo-app/2.0"], Deployit_dictionary['Environments/dict_env']],
}